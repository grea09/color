...
public class IllegalFixer {
    ...
    public static void clean(Problem problem) {
        for (Action action : union(problem.actions, problem.plan.vertexSet())) {
            ...
            fixInconsistent(action, problem);

            fixToxic(action, problem);
        }

        for (Edge edge : new HashSet<>(problem.plan.edgeSet())) {
            fixLiar(edge, problem);
        }
    ...
    private static void fixInconsistent(Action action, Problem problem) {
        Set<Edge> dead = new HashSet<>();
        inconsistentPrecondition(action).stream().filter((precondition) -> (problem.plan.containsVertex(action))).forEach((precondition) -> {
            problem.plan.incomingEdgesOf(action).stream().filter((edge) -> !(edge.labels.isEmpty())).map((edge) -> {
                if (edge.labels.contains(precondition) && edge.labels.contains(-precondition)) {
                    Log.d("⊭\t" + action + " has inconsistent fluent " + precondition + " confirmed by edge " + edge);
                    action.preconditions.remove(precondition);
                    edge.labels.remove(precondition);
                } else if (edge.labels.contains(precondition) && action.preconditions.contains(precondition)) {
                    Log.d("⊭\t" + action + " has inconsistent fluent " + precondition + " saved by edge " + edge);
                    action.preconditions.remove(-precondition);
                    edge.labels.remove(-precondition);
                }
                return edge;
            }).filter((edge) -> (edge.labels.isEmpty())).forEach((edge) -> {
                Log.d("⚰\t" + edge + " is now dead.");
                dead.add(edge);
            });
        });
        action.preconditions.removeAll(inconsistentPrecondition(action));

        inconsistentEffects(action).stream().filter((effect) -> (problem.plan.containsVertex(action))).forEach((effect) -> {
            problem.plan.outgoingEdgesOf(action).stream().filter((edge) -> !(edge.labels.isEmpty())).map((edge) -> {
                if (edge.labels.contains(effect) && edge.labels.contains(-effect)) {
                    Log.d("⊭\t" + action + " has inconsistent fluent " + effect + " confirmed by edge " + edge);
                    action.effects.remove(effect);
                    edge.labels.remove(effect);
                } else if (edge.labels.contains(effect) && action.effects.contains(effect)) {
                    Log.d("⊭\t" + action + " has inconsistent fluent " + effect + " saved by edge " + edge);
                    action.effects.remove(-effect);
                    edge.labels.remove(-effect);
                }
                return edge;
        ...
        action.effects.removeAll(inconsistentEffects(action));
        
        problem.plan.removeAllEdges(dead);
    }

    private static void fixToxic(Action action, Problem problem) { //☣
    ...
    private static void fixLiar(Edge edge, Problem problem) {//☍
