 ...
 * @author antoine
 */
public class PopThreat<F extends Fluent<F, ?>> extends Flaw<F> {
	...
	public PopThreat(F fluent, Action<F, ?> breaker, CausalLink threatened,
			Problem problem) {
...
//            Log.w("Can't demote after goal step !");
			resolvers.add(new Bind<>(threatened.target(), breaker));
		}
		if (threatened.source() != problem.initial) {
//            Log.w("Can't promote before initial step !");
			resolvers.add(new Bind<>(breaker, threatened.source()));
		}
		return resolvers;
	}
	...
	public Set<PopThreat<F>> related(Resolver<F> resolver) {
		Set<PopThreat<F>> result = new HashSet<>();
	...
	public <F extends Fluent<F, ?>> Set<PopThreat<F>> related(CausalLink fragile,
			Action<F, ?> breaker) {
		Set<PopThreat<F>> related = new HashSet<>();
