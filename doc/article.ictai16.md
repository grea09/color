...
The planning problem is defined as a tuple $\mathcal{P} = \langle \mathcal{D}, I, G, \pi\rangle$ <!--C_\mathcal{P},--> where
...
To start, LOLLIPOP computes a safe operator graph (full lines in @fig:experimentplan). Since this plan is nearly complete the main refining function (time markers **1**) receives an agenda with only a few flaws remaining. <!--Then the main refinement function starts (time markers **1**).--> LOLLIPOP adds a causal link from $a$ to satisfy the open condition of the goal step.
...
The measurements exposed in @tbl:results were made with one core of an Intel® Core™ i7-4720HQ with a 2.60GHz clock. <!--The mesurment error was mesured to be $70 ns$.--> We can see an increase of performance in the online runs because of the way they are conducted by LOLLIPOP.
